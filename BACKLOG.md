...
- [ ] 2026-09-12 Actually run `install.sh` and `uninstall.sh` end to end — both are syntax- and shellcheck-clean but have never been executed even once. The agent session that wrote them had `bash install.sh` blocked by a safety classifier, exactly as happened to `vps-webserver`. Run: `sudo VPSSRV_MODULES=web,iperf3 PREFIX=/opt/vps-server-test SERVICE_NAME=vps-server-web-test VPSSRV_HOST=127.0.0.1 VPSSRV_PUBLIC_HTTP_PORT=18080 VPSSRV_PUBLIC_HTTPS_PORT=18443 VPSSRV_CONSOLE_PORT=18090 bash install.sh`, check the service comes up and the summary block is right, then tear it down with the same PREFIX/SERVICE_NAME. Check the port guard too, by holding 18080 first
