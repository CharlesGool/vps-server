...
- [ ] 2026-09-12 Run `install.sh` with the anytls module — first attempt failed before sing-box was installed, because the binary was recorded as `100644` (CIFS ate the mode bit during vendoring) and `install_singbox` tested `-x`. Fixed on both sides: the index now records `100755`, and the test is `-f`. Still unverified past that point: the three renamed constants (unit, config dir, binary path), `refuse_if_upstream_running`, and anytls teardown via `uninstall.sh`. Needs an amd64 host
