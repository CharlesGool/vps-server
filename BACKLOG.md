...
- [ ] 2026-09-12 Click the anytls reset button once after reinstalling — the failure mode is fixed by running the reset in a transient unit via `systemd-run`, and that mechanism is verified on a real host (flags supported, transient unit can write `/etc`, exit codes propagate, `--collect` reaps failed units). The reset itself has still never completed end to end
