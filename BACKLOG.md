...
- [ ] 2026-09-12 Run an actual upgrade over an older install — the helpers were driven individually against synthetic state (a unit file with recorded settings, a node config, a stamp from an older variable set) and all behaved, but `install.sh` has not been run end to end in upgrade mode. Install once, change a setting via env (`VPSSRV_PUBLIC_HTTP_PORT=8080`), re-run, and confirm the summary still shows 8080 and the anytls password is unchanged
