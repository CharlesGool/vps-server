...
> 译自 `DECISIONS.md`（v1.1.2）。如有冲突，以英文版为准。
...
## 2026-09-21 — `prompt_new_settings()` 以显式的 `return 0` 结束，而不是靠循环自然带出退出状态

- **Rejected:** 让函数的退出状态跟着最后一个 `for` 循环自然带出——`install.sh` 里大多数函数都是这样写的。这个循环原本的最后一条语句是裸的 `[ -n "$value" ] && export ...`，于是只要*最后一项*被提示的设置保留默认值，这个判断就是假，这个假值就顺势成了函数本身的返回状态。当它被裸调用时（`prompt_new_settings` 用在一个 `if` 语句体内，本身没有被排除在 `set -e` 之外），这会在最后一次提示结束后悄无声息地杀死整个安装程序——没有报错、没有复制文件、没有打上 `VERSION` 标记、没有重启服务。在 v1.0.4 → v1.1.1 的实际升级中复现过；修复方式是把 export 语句包进 `if`/`fi`，并在末尾加上显式的 `return 0`，让函数的退出状态不再取决于最后被提示的到底是哪一项设置。
- **不要把末尾这行 `return 0`当作看似多余的死代码删掉。** 它就是这次修复本身，不是可有可无的样板代码。
