...
> 譯自 `BACKLOG.md`（v1.1.2）。如有衝突，以英文版為準。
...
- [x] 2026-09-19 由 console 管理連接埠轉發 — 讓 console 把本機上的一個公開 TCP/UDP 連接埠轉發到透過 Tailscale 或區域網路連線的另一台裝置（也就是「這台機器有公網 IP，那台裝置沒有」的情境）。實作方式：每條規則各用一組 iptables DNAT + MASQUERADE，並標上註解以便識別；規則以 JSON 形式持久化，每次服務啟動時都會冪等地重新套用（無論是單純重啟還是重開機，都會從該檔案重播 —— 核心自己的表格本身不會記得任何東西）。`net.ipv4.ip_forward` 會在第一次需要時開啟，並刻意不自動關閉，因為主機上可能還有其他軟體也依賴它。新增 `/portfwd` console 頁面：可新增/啟用/停用/刪除，協定支援 tcp/udp/both，並會檢查公開連接埠是否與這套安裝已經擁有的其他監聽器衝突。已在一個可拋棄的三容器 Docker 測試環境中驗證（而非這台正式主機，因為它本身已經在跑 Docker 自己的 iptables/NAT 狀態 —— 與本檔案中其他所有端到端測試採用相同的理由）：一個接了兩張網路的「vps」容器代表公開/私有網路的分割，一個「target」容器代表 Tailscale/LAN 裝置，一個「client」容器代表公開訪客。已現場確認：TCP 和 UDP 都能端到端轉發；`net.ipv4.ip_forward` 會在第一條規則啟用時自動從 `0` 變成 `1`；停用/啟用會立即切換可達性；連續兩次終止並重新啟動行程，都會從 `portfwd.json` 重新套用，不會產生重複的 iptables 規則；乾淨的 `SIGTERM` 會撤除每條規則在核心中的狀態，同時在磁碟上保留 `enabled: true`，因此重啟後會立刻恢復；公開連接埠衝突（用 console 自己的連接埠測試過）與無效的目標 IP，兩者都會被正確地拒絕並顯示對應訊息。119/119 個單元測試通過，包含新增的 `PortForwardManagerTest`（mock 掉 iptables）與 `PortForwardConsoleTest`（對路由進行實際 HTTP 測試）兩個測試類別。建置在 `feat/portfwd` 分支上；尚未合併、發行或打 tag —— 在那之前，還要等操作者自己完成手動測試。
- [x] 2026-09-21 修復 `install.sh` 在真實升級時悄悄中止的問題 — 由操作者將一台真實主機從 v1.0.4 升級到 v1.1.1 時發現（`v1.1.0` 新增的兩個 `VPSSRV_PORTFWD_*` 設定，是第一次升級真的遇到需要提示的全新設定項）。`prompt_new_settings()` 最後那個 `for` 迴圈以一句裸露的 `[ -n "$value" ] && export "$var=$value"` 結尾；當使用者接受**最後**一個被提示設定項的預設值時，這個測試會為 false，並成為該函式自身的結束狀態 —— 而它是在一個 `if` 區塊裡被裸呼叫的，本身並不豁免於 `set -e`，於是整個安裝程式在最後一個提示之後就直接中止，沒有輸出任何錯誤：沒有複製檔案、沒有蓋上 `VERSION` 戳記、沒有重啟服務，讓機器悄悄卡在舊版本，看起來卻像升級已經完成。已在操作者的真實主機上重現兩次（症狀相同，兩次都是同樣過期的 `ActiveEnterTimestamp`），並用 `bash -x` 追蹤，加上一個獨立重現 `set -e`/`&&`/迴圈互動的 bash 範例，精確地根因了這個問題。修法是把該 export 包進 `if`/`fi`，並在結尾明確加上 `return 0`。以兩種方式驗證：一個獨立的 bash 片段證明了確切的失敗/修復機制，以及在一個可拋棄的 systemd 容器中做的完整實機重現 —— 先安裝真實的 v1.0.4，接著透過真實的 pty（用 `expect`，而不是用管線/非互動式 stdin，那樣會掩蓋這個 bug）驅動一個真正互動式的 `bash install.sh` session 升級到 v1.1.1：未修補的腳本在和操作者遇到的完全同一行中止；修補後的腳本則順利完成，重新蓋上了 `VERSION` 戳記，並重啟了服務。已隨 v1.1.2 出貨。
- [ ] 2026-09-19 參考 vaxilu/x-ui 的做法，支援更多代理協定 — 目前唯一的節點類型是 anytls；使用者預期新增更多協定（例如 vmess/vless/trojan/shadowsocks）不會帶來實質的額外負擔。需要先決定：現有已引入的 sing-box 是否已涵蓋想要的協定，還是需要第二個後端
- [ ] 2026-09-19 為每個節點加上流量統計、流量上限與到期日 — 仿照 x-ui 的節點管理方式：追蹤每個節點的上傳/下載流量，一旦達到流量上限或到期日就自動停用，並在 console 上顯示累計總量
- [ ] 2026-09-19 新增節點的 QR code — 在 console 頁面上，把現有的 anytls 分享連結（以及之後新增的任何節點類型）以可掃描的 QR code 呈現，放在可複製的文字連結旁邊
- [ ] 2026-09-19 模組化安裝程式 — `install.sh` 目前的模組選擇器是一個編號選單（`1`/`2`/`3`/`4` 對應固定的組合，或是用 `VPSSRV_MODULES=web,iperf3,...`）；應把這個互動式提示改成逐項核取清單，讓每個模組（web、iperf3、anytls，以及上面 portfwd/協定相關工作日後落地後的模組）都能獨立勾選，而不是從固定的預設組合中挑一個。`VPSSRV_MODULES` 無論如何都會保留下來，作為無人值守/可寫成腳本的安裝路徑
- [ ] 2026-09-19 網頁版首次執行設定頁面 — `install.sh` 目前預設就已經會隨機產生 console 連接埠與管理員密碼（`VPSSRV_CONSOLE_PORT=0`；`admin_password.txt` 會在首次執行時產生），這部分已經完成。缺少的是：一個瀏覽器設定精靈，讓操作者完全不必在終端機上回答 `install.sh` 的互動式提示（語言、模組、連接埠、驗證方式）。構想：`install.sh` 只做最小限度的隨機化開機並啟動服務，其餘設定（模組、連接埠、驗證方式等）改由 console 中一個首次執行的 `/setup` 頁面，從瀏覽器完成。需要先決定：由瀏覽器驅動的選擇（例如安裝 anytls 模組）該如何回傳成 `install.sh` 原本會執行的 shell 動作
